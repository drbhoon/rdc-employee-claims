    ...
    <Shell title="Admin Dashboard">
      ...
      <section className="card mt-4">
        ...
        <form action={saveClaimType} className="mb-4 grid gap-3 md:grid-cols-6">
          ...
          <div><label>Payment Treatment</label><select name="paymentTreatment"><option value="REIMBURSEMENT">Reimbursement (+)</option><option value="EMPLOYEE_ADVANCE">Employee advance (-)</option></select></div>
        ...
        <table><thead><tr><th>Name</th><th>Payment</th><th>Attachment</th><th>Max Line</th><th>Monthly</th><th>Cost Head</th><th>GL</th><th>Active</th><th>Action</th></tr></thead><tbody>{claimTypes.map((t) => <tr key={t.id}><td>{t.name}</td><td>{t.paymentTreatment === "EMPLOYEE_ADVANCE" ? "Advance (-)" : "Reimbursement (+)"}</td><td>{String(t.attachmentRequired)}</td><td>{t.maxAmountPerLine ? String(t.maxAmountPerLine) : "-"}</td><td>{t.monthlyLimit ? String(t.monthlyLimit) : "-"}</td><td>{t.costHead || "-"}</td><td>{t.glCode || "-"}</td><td>{String(t.isActive)}</td><td><form action={deleteClaimType}><input type="hidden" name="id" value={t.id} /><button className="btn-secondary" type="submit">Delete</button></form></td></tr>)}</tbody></table>
      </section>
      <section className="card mt-4">
        <h2 className="mb-3 font-semibold">Employee Master</h2>
