...
export default async function PaymentsPage({ searchParams }: { searchParams: { from?: string; to?: string; glCode?: string } }) {
  await requirePaymentUploader();
  const [batches, claimTypes] = await Promise.all([
    prisma.paymentUploadBatch.findMany({ orderBy: { uploadedAt: "desc" }, take: 10 }),
